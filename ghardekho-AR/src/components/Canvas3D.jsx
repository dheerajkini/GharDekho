...
function FurnitureMesh({ item, selected, onSelect, onDragEnd, setOrbitEnabled, rW, rL }) {
  ...
  const hitFloor = useCallback((cx, cy) => {
    ...
    return {
      x: Math.max(-rW/2 + item.w3d/2, Math.min(rW/2 - item.w3d/2, hit.x)),
      z: Math.max(-rL/2 + item.d3d/2, Math.min(rL/2 - item.d3d/2, hit.z)),
    };
  }, [camera, gl, rW, rL, item.w3d, item.d3d]);
  ...
  return (
    <group
      ...
      onPointerUp={onUp}
    >
      {/* key forces geometry recreate on rotate or color change */}
      <Shape
        key={`${item.w3d}-${item.d3d}-${item.rKey||0}`}
        w={item.w3d}
        d={item.d3d}
        color={item.color}
        sel={selected}
      ...
      {selected && (
        <mesh position={[0,0.015,0]} rotation={[-Math.PI/2,0,0]}>
          <ringGeometry args={[
            Math.max(item.w3d,item.d3d)*0.62,
            Math.max(item.w3d,item.d3d)*0.70, 48
          ]} />
...
export default function Canvas3D({ room, sharedItems, onItemsChange }) {
  ...
  // ── 2D → 3D sync (preserves w3d, d3d, rKey) ──────────────────
  useEffect(() => {
    if (!sharedItems) { setItems3d([]); return; }
    setItems3d(prev =>
      sharedItems.map(s => {
        const ex   = prev.find(p => p.id === s.id);
        const meta = META[s.name] || { w:1, d:1 };
        const { x3d, z3d } = to3D(s, rW, rL);
        return {
          id:    s.id,
          name:  s.name,
          color: s.color,           // always take latest color
          x3d,   z3d,
          w3d:  ex ? ex.w3d  : meta.w,  // preserve rotated size
          d3d:  ex ? ex.d3d  : meta.d,
          rKey: ex ? ex.rKey : 0,        // preserve rotate key
        };
      })
    );
  }, [sharedItems, rW, rL]);
  ...
  // ── ROTATE — swaps w3d/d3d and bumps rKey to force re-render ──
    ...
    setItems3d(prev =>
      prev.map(i => {
        ...
        return {
          ...
          w3d:  i.d3d,
          d3d:  i.w3d,
          rKey: (i.rKey || 0) + 1,  // ← THIS forces Three.js to recreate geometry
        };
      })
    );
  };

  // ── COLOR — instant update in 3D + sync to 2D ────────────────
  ...
  // ── Styles ────────────────────────────────────────────────────
  const S = {
    sb:  { background:"rgba(10,10,28,0.95)", borderRight:"1px solid rgba(108,92,231,0.15)", width:"185px", flexShrink:0, overflowY:"auto", padding:"10px", display:"flex", flexDirection:"column", gap:"8px" },
    sec: { background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:"10px", padding:"10px" },
    h:   { fontSize:"0.65rem", textTransform:"uppercase", letterSpacing:"1px", color:"#6c5ce7", fontWeight:"800", marginBottom:"8px" },
    tb:  { padding:"6px 12px", borderRadius:"7px", cursor:"pointer", fontSize:"0.73rem", fontWeight:"600", transition:"all 0.15s" },
  };

  return (
    <div style={{display:"flex", height:"100%", background:"#07071a"}}>
          ...
          {CATALOG.map(p=>(
            ...
            >
              ...
              <div>
                ...
                <div style={{color:"#444",fontSize:"0.62rem"}}>{p.length}×{p.width}ft</div>
              </div>
            </button>
          ))}
        </div>
      ...
      <div style={{flex:1, display:"flex", flexDirection:"column"}}>
        ...
        <div style={{
          ...
          <div style={{flex:1}} />
          <span style={{fontSize:"0.63rem",color:"#2a2a4a"}}>
            🖱️ Drag furniture → move &nbsp;|&nbsp;
          ...
          {room.length && room.width ? (
            <Canvas
              ...
              style={{
                ...
                background:"linear-gradient(160deg,#0d0d1f 0%,#1a1a2e 100%)",
              }}
            ...
            >
              ...
              />
              <pointLight position={[0,rH*0.8,0]}    intensity={0.25} color="#a29bfe" />
              <pointLight position={[rW/2,rH*0.5,0]} intensity={0.15} color="#00b894" />
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%"}}>
              ...
              <div style={{color:"#a0a0b0"}}>Set room dimensions to see 3D</div>
            </div>
          )}
        </div>
      </div>
